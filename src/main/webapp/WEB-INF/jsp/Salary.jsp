...
//查询
$("#myTable").datagrid({
	url:"<%=request.getContextPath()%>/salaryanswer",
	columns:[[
...
pagination:true,//开启分页
	 pageList:[3,10,15,25,50,100], //初始化页面大小选择列表
...
function deleteBys(){
	...
	function openUpdateBy(id){
		//alert(id)
		$.ajax({
			url:"<%=request.getContextPath() %>/querySalaryById",
			type:"post",
			data:{"id":id},
...
*/
	...
	function add(){
		$("#myForm").form("submit",{
			url:"<%=request.getContextPath() %>/addAsalary",
			success:function(){
